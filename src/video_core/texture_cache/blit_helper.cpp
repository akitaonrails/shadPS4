...
#include "video_core/host_shaders/ms_depth_to_color_frag.h"
...
BlitHelper::~BlitHelper() {
    ...
    device.destroy(ms_depth_to_color_frag);
...
void BlitHelper::ReinterpretMsDepthAsColor(u32 width, u32 height, u32 num_samples,
                                           vk::Format src_pixel_format, vk::Format dst_pixel_format,
                                           vk::Image source, vk::Image dest) {
    // Source view: MSAA depth, sampled-image aspect.
    const vk::ImageViewUsageCreateInfo depth_usage_ci{.usage = vk::ImageUsageFlagBits::eSampled};
    const vk::ImageViewCreateInfo depth_view_ci = {
        .pNext = &depth_usage_ci,
        .image = source,
        .viewType = vk::ImageViewType::e2D,
    ...
    // Destination view: 1-sample color attachment.
    const vk::ImageViewUsageCreateInfo color_usage_ci{
    ...
    state.color_attachments[0].image_view = color_view;
    ...
    // Pipeline key: destination (color) format + MSAA sample count of the source.
    // src_msaa is always true in this direction.
    const MsPipelineKey key{num_samples, dst_pixel_format, true};
    auto it = std::ranges::find(ms_depth_to_color_pl, key, &MsPipeline::first);
    if (it == ms_depth_to_color_pl.end()) {
        CreateMsDepthToColorPipeline(key);
        it = --ms_depth_to_color_pl.end();
    }
    cmdbuf.bindPipeline(vk::PipelineBindPoint::eGraphics, *it->second);
...
void BlitHelper::CreateShaders() {
    ...
    ms_depth_to_color_frag = Vulkan::Compile(HostShaders::MS_DEPTH_TO_COLOR_FRAG,
                                             vk::ShaderStageFlagBits::eFragment, device);
...
void BlitHelper::CreateMsDepthToColorPipeline(const MsPipelineKey& key) {
    // Source is MSAA depth, destination is a 1-sample color attachment.
    // Rasterization samples therefore match the COLOR attachment (1x), not
    // the source — the shader does the sample fetch itself via texture2DMS.
    const vk::PipelineInputAssemblyStateCreateInfo input_assembly = {
        .topology = vk::PrimitiveTopology::eTriangleList,
    };
    const vk::PipelineMultisampleStateCreateInfo multisampling = {
    ...
    ms_depth_to_color_pl.emplace_back(key, std::move(pipeline));
}
